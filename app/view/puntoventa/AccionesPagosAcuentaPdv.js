Ext.define('megafilmperu.view.puntoventa.AccionesPagosAcuentaPdv', {
    ...
    onClickGuardarPagoAcuenta:function(btn){
      ...
      if(__store){
        ...
        Ext.Ajax.request({
            url :megafilmperu.util.Rutas.facturacionAgregarPagosAcuenta,
            params:{
              idfacturacion :  Ext.ComponentQuery.query('#idfacturapc')[0].getValue() ,
              vjsondetalle  :  JSON.stringify(__jsondata)
             },
            success:function(response){
                  __data = Ext.JSON.decode(response.responseText);
                  if(__data.error!=0){
                    megafilmperu.util.Util.showToast('Registrado');
                    __view.close();
