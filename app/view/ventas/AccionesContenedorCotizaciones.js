Ext.define('megafilmperu.view.ventas.AccionesContenedorCotizaciones', {
    ...
    onClickRefrescarListadoCotizaciones: function () {
        _store = Ext.ComponentQuery.query('#dgvVentas')[0].getStore();
        _store.load(1);
    },
    onClickImprimirPDFCotizacion: function (btn) {
        ...
        if (_record) {
            var objrpt = window.open( megafilmperu.util.Rutas.rptImprimirCotizacion+ 'id='+ _record.get('vid'), "", "width=700,height=900");
    ...
    onClickConfirmarCotizacion:function(btn){
      me = this;
      r =  Ext.ComponentQuery.query('#dgvVentas')[0].getSelectionModel().getSelection()[0];
      if (r) {
         me.getView().mask('..... confirmando datos');
          Ext.Ajax.request({
            url :megafilmperu.util.Rutas.confirmarVentaCotizacion,
            params:{
              idcoti : r.get('vid')
            },
            success:function(response){
                oj =response.responseText;
                me.getView().unmask();
                Ext.ComponentQuery.query('#dgvVentas')[0].getStore().reload();
                Ext.ComponentQuery.query('#dgvDetalleCotizacion')[0].getStore().load();
            }
          });
      } else {
    ...
    onClickVisorClienteCotizaciones: function (btn) {
       /* var _record = this.lookupReference('dgvVentas').getSelectionModel().getSelection()[0];
        if (_record) {
            ...
            var _win = Ext.create('megafilmperu.view.ventas.VisorClienteCotizacion');
...
*/
            var _win = Ext.create('megafilmperu.view.ventas.VisorProductoCotizacion');
            _win.show(btn, function () {}, this);
        /*} else {
            Ext.Msg.alert("Error", "Tiene que seleccionar un cliente");
            return false;
    ...
    onClickVisorVendedorCotizaciones: function (btn) {
        ...
        if (_record) {
            ...
            var _win = Ext.create('megafilmperu.view.ventas.VisorVendedorCotizacion');
            _win.show(btn, function () {}, this);
        /*} else {
            Ext.Msg.alert("Error", "Tiene que seleccionar un cliente");
            return false;
