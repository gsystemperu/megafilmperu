...
class CotizacionController extends Controller
{
    ...
    public function productolistaporclienteAction(){
         ...
         if($request->isGet() ==true)
         {
              ...
              //$data = array($vCodigo,$vDescripcion,$vIdCliente);
    ...
    public function agregarcotizacionAction(){
         ...
         if($request->isPost() ==true)
         {
              ...
              $data = array($vId,$vFecha,$vIdCliente,$vUsuario,$vJsonDetalle, $vFormaPago,$vModoEntrega,$vFechaValidoHasta,$vcomentario);
              //print_r($data);die();
    ...
    public function agregarclienteAction(){
         ...
         if($request->isPost() ==true)
         {
              ...
              $vPorcentajeGerencia = $request->getPost('porcentajegerencial');

              $data = array( $vId,$vPaterno,$vMaterno,$vNombre,$vSexo,$vFechaNaci,$vidDoc,$vNumDoc,$vNumRuc,$vDireccion,$vTelefono,$vCelular,$vUsuario,'','',$vPrecio ,$vPorcentajeGerencia);
     ...
     public function agregarclientevialistadoAction(){
         ...
         if($request->isPost() ==true)
         {
              ...
              $data = array($vId,$vPaterno,$vMaterno,$vNombre,$vSexo,$vFechaNaci,$vidDoc,$vNumDoc,$vNumRuc,$vDireccion,$vTelefono,$vCelular,$vUsuario,$vCorreo,$vProvincia,$vPrecio,$vPorcentajeGerencia);
              $jsonData             = Cotizacion::agregarCliente($data);
              $response->setContentType('application/json', 'UTF-8');
              $response->setContent(json_encode($jsonData[0], JSON_NUMERIC_CHECK));
              return $response;
   ...
   public function unidadmedidaactualizarAction(){
         ...
         if($request->isPost() ==true)
         {
              $vId =  $request->getPost('idumed');
              $vDescripcion = $request->getPost('descripcion');
              $vAbreviatura =  $request->getPost('abreviatura');
   ...
   public function formapagoeliminarAction(){
       ...
       if($request->isPost() ==true)
       {
            ...
            $data       = array($vIdfpago);
            $jsonData   = Cotizacion::eliminarFormaPago();
            $response->setContentType('application/json', 'UTF-8');
            $response->setContent(json_encode($jsonData, JSON_NUMERIC_CHECK));
            return $response;
       }
  ...
  public function listarCotizacionesParaFacturarAction(){
       ...
       if($request->isGet() ==true)
       {
            ...
            if($vDesde!='' && $vHasta !=''){
                ...
                $data = array();
                $jsonData     = Cotizacion::listarCotizacionesParaFacturar();
            }
