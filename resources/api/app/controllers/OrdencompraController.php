...
class OrdencompraController extends Controller
{
  ...
  public function modificarAction()
    ...
    *
    ...
    public function anularconfirordencompraAction(){
        $request        = new \Phalcon\Http\Request();
        $response       = new \Phalcon\Http\Response();
        $format         = new FuncionesHelpers();
        if($request->isPost() ==true)
        {
             $data = array($request->getPost('idordencompra'),$request->getPost('usuario'));
             $jsonData = OrdenCompra::anularConfirOrdenCompra($data);
             $response->setContentType('application/json', 'UTF-8');
             $response->setContent(json_encode($jsonData[0], JSON_NUMERIC_CHECK));
             return $response;
        }
   ...
   public function anularfacturaordencompraAction(){
    $request        = new \Phalcon\Http\Request();
    $response       = new \Phalcon\Http\Response();
    $format         = new FuncionesHelpers();
    if($request->isPost() ==true)
    {
         $data = array($request->getPost('idordencompra'),$request->getPost('usuario'));
         $jsonData = OrdenCompra::anularFacturaOrdenCompra($data);
         $response->setContentType('application/json', 'UTF-8');
         $response->setContent(json_encode($jsonData[0], JSON_NUMERIC_CHECK));
         return $response;
    }
