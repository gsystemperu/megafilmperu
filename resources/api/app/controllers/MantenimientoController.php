...
class MantenimientoController extends Controller
{
    ...
    public function almacenseccioneslistarAction(){
         ...
         {    $idalmacen = $request->get('id');
              $jsonData  = Almacen::ListarSecciones($idalmacen);
              $response->setContentType('application/json', 'UTF-8');
              $response->setContent($jsonData);
              return $response;
         }
    ...
    public function medidasactualizarAction(){
         ...
         if($request->isPost() ==true)
         {
              ...
              $descripcion = $request->getPost('descripcion');
              $ancho  = $request->getPost('ancho');
              $largo  = $request->getPost('largo');
              $data = array($idmedida,$descripcion,$ancho,$largo);
 ...
 public function tipoproductoactualizarAction(){
    ...
    public function tarifaeliminarAction(){
     ...
     //@ Tabla Moneda
    //========================
    public function monedalistarAction(){
        $request        = new Phalcon\Http\Request();
        $response       = new \Phalcon\Http\Response();
        if($request->isGet() ==true)
        {
             $jsonData  = Empresa::monedalistar();
             $response->setContentType('application/json', 'UTF-8');
             $response->setContent($jsonData);
             return $response;
        }
